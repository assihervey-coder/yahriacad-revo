...
Task ID: 21
Agent: Super Z (agent principal)
Task: Exécution des 3 sprints M1-M6 (preuve physique harnais, collaboration opérée, robustesse routeur) — chaque jalon implémenté, testé, E2E-vérifié, commité et poussé.

Work Log:
- Sprint 1 — M1 : PUBLISHED_THRESHOLDS (r ≥ 0,55, RMSE ≤ 8 °C, err ≤ 15 °C, ≥ 2 relevés) + measuredThresholdVerdict + IC 95 % de la pente (Student) dans calibration.ts ; CLI calibrate_measured.ts versionné (révision SHA-256 déterministe via canonisation récursive — le replacer-array JSON écrase les propriétés imbriquées, banni), empreinte dataset, dataKind demo|measured, REFUS hors seuil (exit 1) ; jeu de DÉMONSTRATION fidèle au protocole (6 cartes layout fixe jitter 0→10 mm × 3 ambiances, graines publiées) — 2 cartes seules = support dégénéré (r=1 par construction, enseignement documenté) ; coefficients publiés : nexus-core conforme (r 0,792, RMSE 4,2 °C, révision 4d4706ce…), nexus-iot (r 0,487) et nexus-rf (noyau ininformatif, sources chaudes > 12 mm) refusés par le garde-fou ; protocole d'acquisition complet docs/protocole-acquisition-cartes-mesurees.md ; +6 assertions.
- Sprint 1 — M2 : carte UI « Droite de calage mesurée » à côté de la corrélation simulation (badge démonstration|campagne, pente ± IC, intercept, r, RMSE, prédiction MESURÉE du placement courant, note de portée : refs, ambiance, relevés, seuils, révision, dataset) ; état « en attente » honnête sinon ; coefficients reproductibles depuis le dépôt (CLI + JSON versionné) ; E2E : prédiction mesurée 23,5 °C vs simulation 18,0 °C affichées côte à côte, zéro erreur console.
- Sprint 2 — M3 : use-db.sh bascule COMPLÈTE (provider + DATABASE_URL .env + client) — l'ancienne laissait le serveur silencieusement sur SQLite ; archive nexus-db-archive v1 (db-dump/db-restore : cuid + horodatages préservés, SHA-256, --wipe ordonné FK) ; migrate-sqlite-to-pg.sh 6 étapes — exécutée : 2 projets/37 runs/5 edits transférés ; pg_backup_restore_test.ts 10/10 (fidélité bit-à-bit floats/ms/acteurs) ; page.tsx : journal chargé AU DÉMARRAGE À FROID ; docs/migration-sqlite-postgresql.md ; E2E deux sessions : A conçoit 27/28 + nudges, B voit run + journal, kill -9 → restart → intact. DÉCOUVERTE : variable shell DATABASE_URL pré-M3 écrasait .env — le serveur tournait encore sur SQLite ; re-opéré sur PG avec sonde de marquage PG-only.
- Sprint 2 — M4 : modèle User (rôle String — réversibilité provider préservée), session cookie httpOnly, rôle relu en base à chaque requête ; gardes 401/403 sur routes PERSISTANTES (edits/projects/runs/users) — les routes de PUR CALCUL (SSE live, agents LLM, MCP) restent ouvertes (le smoke d'infra les exige, rien n'y est écrit) ; journal : acteur de SESSION fait foi (body ne peut pas usurper) ; /api/auth/login|logout|me + /api/users (admin CRUD, pas soi-même) ; en-tête : sélecteur de compte + badge de rôle ; refus rendus visibles ([JOURNAL REFUSÉ 403]) ; test-roles.ts 12/12 ; E2E : nudge ingénieur → « Théo Moreau (ingenieur) », nudge lecteur → refus visible, aucune entrée.
- Sprint 3 — M5 : A* généralisé multi-couches DERRIÈRE drapeau allLayers (passes P1.1 bit-à-bit sinon — bug évité : padNetL1 hors bornes bloquait les couches internes, bornes y keepout restaurées) ; passe 2b ADDITIVE « paire de couches supplémentaire » (plans L2/L3 corridor de secours des nets EN ÉCHEC, pénalité plane 0,6, minimisation vias de secours viaCost 60, buildPlane exclut le cuivre signal) ; passe 2c ADDITIVE « excavation de keepout » (anneau extérieur, noyau keepSoft interdit, accord tracé Route.keepoutExcavated) ; rip-up étendu OPT-IN (14 rounds, 3 bloquants, relance perturbée strictement locale) ; pile 6 couches paramétrable (setLayers 2|4|6, UI, exports Gerber/ODB++ génériques) ; DoD : 4c 28/28 (2 nets via paire supplémentaire), rip-up étendu 27/28, 6c 28/28, 2 couches bit-exact 26/28@77 vias, plans intacts.
- Sprint 3 — M6 : viewer.multiRefs (Maj+clic toggle, état relu à chaque clic) + surgicalMoveGroup (un bloc, un re-routage, UNE entrée de journal PAR composant) ; flèches groupées, Escape purifie, badge « N sélectionné(s) » ; src/lib/engine/obj.ts (dalle FR4 + volumes rotation inclus, convention axes, 1 unité = 1 mm) + bouton « OBJ 3D (positions mécaniques) » ; +5 assertions/projet ; E2E : 5 sélectionnés → flèche → 5 entrées « déplacement groupé (5 composants) » attribuées à l'ingénieur, OBJ téléchargé.
- Incidents résolus : serveur zombie EADDRINUSE (kill arbre complet bun/next/tee) ; client Prisma périmé en mémoire après régénération (cache .next vidé) ; hook pre-push SSE bloqué par le garde M4 (routes de calcul libérées, smoke repassé).

Stage Summary:
- 6 jalons M1-M6 implémentés, testés et poussés : 6cf8691 (M1+M2), 02b8e22 (fix traçabilité), c688cfa (M3), 6da0682 (M4), 6d59c74 (M5), d92bc9c (M6) — hook pre-push vert à chaque push (moteur 240+ assertions + SSE).
- Position honnête maintenue : coefficients mesurés = DÉMONSTRATION synthétique (campagne physique M1 en attente, protocole livré) ; PG opéré en auto-hébergé (instance managée = même procédure avec URL distante) ; lecteur/ingénieur/admin appliqués serveur.
- Restant pour un audit V5 : annexer M1-M6 au registre avec ces preuves.
